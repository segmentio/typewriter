...
import { resolveRelativePath, runScript, Scripts, TrackingPlanConfig, verifyDirectoryExists } from '../config';
import { supportedLanguages } from '../languages';
import { toCommandConfig } from '../telemetry';
import { Mode } from '../telemetry/segment';
...
export default class Build extends BaseCommand {
  ...
  private async clearGeneratedFiles(trackingPlanConfig: TrackingPlanConfig): Promise<void> {
  ...
  public async run(): Promise<void> {
    ...
    const languageGenerator = supportedLanguages.find((lang) => lang.id === language);
    ...
    for (const plan of trackingPlans) {
      if (plan.rules === undefined || plan.rules.length === 0) {
        CliUx.ux.action.status = chalk.yellow(`No rules found for ${plan.name}. Skipping...`);
        continue;
      }
      ...
      try {
        ...
        const workspacePlan = this.workspaceConfig.trackingPlans.find((tp) => tp.id === plan.id);
        ...
        for (const [filename, contents] of files.entries()) {
          ...
          const filepath = resolveRelativePath(this.configPath, workspacePlan!.path, fileWithExtension);
    ...
    this.segmentClient.buildCommand({
      properties: {
        buildTime: process.hrtime(startTime),
        config: toCommandConfig(this.workspaceConfig, this.tokenMetadata!.method),
