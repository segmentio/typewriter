...
import { resolveRelativePath, TrackingPlanConfig, verifyDirectoryExists } from '../config';
...
export async function loadTrackingPlans(
  ...
  for (const trackingPlanConfig of trackingPlanConfigs) {
    ...
    if (forceUpdate || previousTrackingPlan === undefined) {
      try {
        newTrackingPlan = await fetchTrackingPlan(trackingPlanConfig.id, apiToken);
      } catch (error) {
        const errorMessage = isWrappedError(error) ? error.description : 'API request failed';
